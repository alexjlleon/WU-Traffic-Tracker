(function () {
  ...
  // --- Identity: link this browser to a real person after a form submit ---
  //   wuIdentify({ name: 'Jane Doe', email: 'jane@example.com', phone: '555-1234' });
  var IDENTITY_KEY = 'wu_identity';
  var identifyEndpoint = endpoint.replace(/\/api\/track$/, '/api/identify');

  function sendIdentity(fields, returning) {
    var sid = null;
    try { sid = localStorage.getItem('wu_session_id'); } catch (e) {}
    if (!sid) return;
    try {
  ...
  window.wuIdentify = function (fields) {
    ...
    sendIdentity(fields, false);
  };

  // Re-link on return visits so later sessions still map to this person.
  try {
    var _saved = localStorage.getItem(IDENTITY_KEY);
    if (_saved) {
      var _f = JSON.parse(_saved);
      if (_f && _f.email) sendIdentity(_f, true);
    }
  } catch (e) {}
